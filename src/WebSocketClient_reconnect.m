classdef WebSocketClient_reconnect < handle
    ...
    methods
        function obj = WebSocketClient_reconnect(URI,varargin)
        ...
        function send(obj,message)
            ...
            if ~obj.Status;
                while(obj.Status == 0)
                    disp("trying to open connection")
                    obj.open()
                    pause(0.1)
                end
